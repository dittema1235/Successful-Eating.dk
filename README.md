...
- Freebie-knapperne på forsiden, `/gratis-guide` og `/sulteneller` åbner den officielle Systeme.io-formular (25559807). Scriptet indlæses først efter klik i et separat iframe-dokument, så automatisk åbning og formularens lukkestatus ikke påvirker resten af siden. Formular og e-maillevering administreres i Systeme.io; URL'er findes i `src/data/guide.ts`. Knappen fungerer som direkte link til formularen uden JavaScript. Ved skift af formular skal også ID'erne i `GuidePopup.astro`, `public/guide-embed.js` og scriptets tilladelse i `public/_headers` opdateres.
- Verificeret 24. september 2026: Den fejlkonfigurerede “Nej tak”-knap (`button-86abbc21`) er fjernet i både Systeme.io-editoren og den offentlige formular. Formularens kryds lukker den indlæste popup; hjemmesidens “Luk”-knap vises under indlæsning og ved fejl. Popuppen kan åbnes igen på mobil og computer uden at sende en tilmelding. Indsendelse og e-maillevering er ikke afprøvet med en rigtig tilmelding.
- Formularen på Systeme.io-siden `44743398` er gjort kompakt: det store topbillede er fjernet, teksten og afstanden er forkortet, og navn, e-mail samt tilmeldingsknap er bevaret. Hjemmesidens popup er tilpasset formularens bredde og højde. Den er visuelt kontrolleret uden indvendig scroll ved 1440×900, iPhone 13, 375×667 og 320×568. Ændringer i formularens indhold vedligeholdes i Systeme.io og skal koordineres med størrelserne i `GuidePopup.astro`.
