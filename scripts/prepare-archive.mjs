...
await writeFile('src/data/archive.json', JSON.stringify(posts));
await writeFile(
  'public/search-index.json',
  JSON.stringify(
    posts.map(({ slug, title, description, topic, published }) => ({
      slug,
      title,
      description,
      topic,
...
await writeFile('docs/editorial-changes.json', JSON.stringify(edits, null, 2));
console.log(`Prepared ${posts.length} articles; ${edits.length} targeted editorial corrections.`);
